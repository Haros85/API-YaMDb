...
class TestDockerfileCompose:
    ...
    def test_dockerfile_compose(self):
        try:
            with open(f'{os.path.join(settings.BASE_DIR, "docker-compose.yaml")}', 'r') as f:  # noqa: E501
        ...
        assert re.search(r'build:\s+\.', docker_compose), \
            'Проверьте, что добавили сборку контейнера из Dockerfile в файл docker_compose'  # noqa: E501
